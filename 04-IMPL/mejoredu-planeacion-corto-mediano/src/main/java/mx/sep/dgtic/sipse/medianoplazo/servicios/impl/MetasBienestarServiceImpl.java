...
public class MetasBienestarServiceImpl implements MetasBienestarService {
	...
	@Inject
	ValorLineaBaseRepository valorLineaBaseRepository;
	@Inject
	SerieMetaRepository serieMetaRepository;
	@Inject
	...
	public List<PeticionMetasBienestarDTO> consultarPorIdEstrucura(Integer idEstructura) {
		var metasBienestar = new ArrayList<PeticionMetasBienestarDTO>();


		List<Meta> metas = metaRepository.find("estructura.idEstructura=?1 and ixTipo=?2", idEstructura,1).list();
	...
	public RespuestaGenerica modificar(PeticionMetasBienestarDTO peticion) {
		...
		if(!ObjectUtils.isEmpty(peticion.getAplicacionMetodo().get(0).getIdAplicacion())){
			Log.info("peticion.getAplicacionMetodo().get(0).getIdAplicacion() " + peticion.getAplicacionMetodo().get(0).getIdAplicacion());
			AplicacionMetodo aplicacionMetodo = aplicacionMetodoRepository.findById(peticion.getAplicacionMetodo().get(0).getIdAplicacion());
		...
		if(!ObjectUtils.isEmpty(peticion.getValorLineaBase().get(0).getIdLinea())){
			Log.info("peticion.getValorLineaBase().get(0).getIdLinea() " + peticion.getValorLineaBase().get(0).getIdLinea());
		...
		if(!ObjectUtils.isEmpty(peticion.getSerieHistorica().size())){
			Log.info("peticion.getSerieHistorica().get(0).getIdSerie() " + peticion.getSerieHistorica().get(0).getIdSerie());
	...
	@Override
	@Transactional
	public RespuestaGenerica eliminar(Integer idMetaBienestar, String cveUsuario) {

		Log.info("Iniciando Transacción de eliminar");
		Log.info("idMetaBienestar  " + idMetaBienestar);
