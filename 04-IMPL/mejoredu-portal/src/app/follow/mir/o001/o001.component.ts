...
import { O001Service } from '@common/services/seguimientoMirFid/o001.service';
...
export class O001Component {
  ...
  constructor(
    ...
    private o001Service: O001Service,
  ...
  ngOnInit(): void {
    ...
    this.o001Service
      .consulta001(this.yearNav, this.cveUsuario, this.currentSemester)
      .subscribe((response) => {
  ...
  submit(): void {
    ...
    this.o001Service.postActividado001(request).subscribe((response: any) => {
  ...
  submit2(): void {
    ...
    this.o001Service.postMetao001(requeest2).subscribe((response: any) => {
      if (response.codigo == 200) {
        this._alertService.showAlert('Se guardó correctamente.');
      }
    });
  ...
  private mapRequest() {
    ...
    return {
      idAnhio: parseFloat(this.yearNav),
      periodo: this.currentSemester,
      cveUsuario: this.dataUser.cveUsuario,
      indicador: {
        nombre: form.nombreIndicador,
        definicion: form.definicion,
        ixTipoValorMeta: parseFloat(form.tipoValorMeta),
        indicadorPEF: parseFloat(form.idIndicadorPEF),
        ixTipoFormula: parseFloat(form.idTipoFormula),
        tipoFormula: form.idTipoFormula == 1 ? '(A/B) * 100' : 'Otro',
        ixEstatusAvance: parseFloat(form.idEstatusAvance),
      },
      ciclo: {
      ...
      cicloAjustado: {
        ...
        denominador: parseInt(form.denominador2),
      },
    };
  }
  ...
  private mapRequest2() {
    ...
    return {
      ...
      otrasMetas: {
        ...
        numerador: parseFloat(form.numeradorOtrasMetas),
        indicador: parseFloat(form.indicadorOtrasMetas),
      },
      cicloAjustado: {
        meta: parseFloat(form.metaEsperadaOtrasMetasCicloAjustado),
        numerador: parseFloat(form.numeradorOtrasMetasCicloAjustado),
        indicador: parseFloat(form.indicadorOtrasMetasCicloAjustado),
        ...
        denominador: form.denominadorOtrasMetasCicloAjustado,
      },
    };
  }
